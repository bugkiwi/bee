...
export class CodexProvider extends BaseProvider {
	...
	constructor(
		opts: { model?: string; timeoutMs?: number; useRtk?: boolean } = {},
	) {
		super();
...
export function parseCodexLine(raw: string, provider: string): ProviderEvent {
	const timestamp = new Date().toISOString();
	if (raw.startsWith("Reading additional input from stdin")) {
		return {
			provider,
			type: "system",
			raw,
	...
	try {
		const parsed = JSON.parse(raw) as CodexLinePayload;
