...
import { generateTaskId, generateId } from "../utils/id.ts";
...
Output the Task JSON now:`;
    ...
    const raw = await this.callClaude(PLANNER_SYSTEM_PROMPT, prompt);
  ...
  async fromSkeletonSpec(goal: string, provider?: string): Promise<PlanSkeleton> {
    const now = new Date().toISOString();
...
Generate the skeleton plan array now:`;
    ...
    const raw = await this.callClaude(SKELETON_SYSTEM_PROMPT, prompt);
    ...
    if (!parsed.success) {
      ...
      throw new ZodValidationError(issues.map((i) => i.path.join(".") + ": " + i.message).join(", "));
    }
...
Generate leaf tasks array now (max 10):`;
    ...
    const raw = await this.callClaude(LEAF_TASK_SYSTEM_PROMPT, prompt);
    const json = extractJsonArray(raw);
    ...
    for (const item of json as unknown[]) {
      const withId = { ...((item as object) ?? {}), task_id: generateTaskId(), status: "pending", created_at: now, updated_at: now };
...
Write the handoff summary paragraph now:`;
    ...
    const summary = await this.callClaude(HANDOFF_SYSTEM_PROMPT, prompt);
    return summary.trim();
  }
  ...
  async buildAskPlan(goal: string, provider?: string): Promise<AskPlan> {
    ...
    for (const skNode of skeleton.nodes) {
      ...
      if (await this.shouldDecompose(node, provider)) {
...
Should this phase be split into sub-phases?`;
    ...
    try {
      const raw = await this.callClaude(DECOMPOSE_CHECK_SYSTEM_PROMPT, prompt);
      const json = extractJson(raw) as { decompose?: boolean } | null;
      return typeof json === "object" && json !== null && json.decompose === true;
    } catch {
...
Generate the sub-phases array now:`;
    ...
    const SubNodeSpecSchema = z.array(
      z.object({
        title: z.string(),
    ...
    const raw = await this.callClaude(SUB_NODE_SYSTEM_PROMPT, prompt);
    const json = extractJsonArray(raw);
  ...
  estimateCost(nodeCount: number): string {
    ...
    const high = (nodeCount * 0.40).toFixed(2);
