...
import type {
	PlanSkeleton,
	SkeletonNode,
	SkeletonProgressEvent,
} from "../types/skeleton.ts";
...
export class AgentLoop {
	...
	async runSkeleton(
		...
		const skeleton = await this.planner.fromSkeletonSpec(
			goal,
		...
		if (opts.onSkeletonReady) {
			...
			if (!proceed) {
				...
				console.log(
					chalk.gray(`  Resume with: bee resume --skeleton ${skeleton.id}`),
				);
		...
		await skeletonLogger.log({
			type: "skeleton:start",
			data: { goal, nodeCount: skeleton.nodes.length },
		});
		...
		for (const node of skeleton.nodes) {
			...
			try {
				...
				console.error(
					chalk.gray(
						"  Inspect the saved skeleton state in .bee/state/ before retrying.",
					),
				);
			...
			opts.onProgress?.({
				...
				nodeId: node.id,
				elapsed,
				summary: handoffContext,
			});
			await skeletonLogger.log({
				type: "node:done",
				nodeId: node.id,
				data: { elapsed, summary: handoffContext },
			});
		...
		try {
			...
			if (opts.onPlanReady) {
				...
				if (!proceed) {
					...
					console.log(
						chalk.yellow(
							`\nRun aborted. Plan saved: .bee/plans/ask-${plan.id}.json`,
						),
					);
					throw new UserAbortError();
				}
			...
			for (const node of plan.root_nodes) {
				handoffContext = await this.runAskNode(
					plan.id,
					node,
					handoffContext,
					session.active_provider,
	...
	private async runAskNode(
		...
		let nextHandoffContext = handoffContext;
		...
		try {
			if (node.sub_nodes && node.sub_nodes.length > 0) {
				...
				for (const sub of node.sub_nodes) {
					nextHandoffContext = await this.runAskNode(
						planId,
						sub,
						nextHandoffContext,
						provider,
				...
				const noopLogger: {
					log: (entry: Omit<SkeletonLogEntry, "ts">) => Promise<void>;
				} = {
					log: async (_entry) => {},
				};
	...
	private async runNode(
		...
		const leafTasks = await this.planner.generateLeafTasks(
			node,
			handoffContext,
			provider,
		);
		...
		for (let i = 0; i < leafTasks.length; i++) {
			...
			console.log(
				chalk.gray(`  • [${i + 1}/${leafTasks.length}] ${leaf.goal}`),
			);
		...
		const completedDesc = completedGoals
			.map((g, i) => `${i + 1}. ${g}`)
			.join("\n");
		const summary = await this.planner.generateHandoffSummary(
			node.title,
			completedDesc,
			provider,
		);
	...
	private async runCriterionCheck(
		...
		if (checkCommand(cmd) === "confirm") {
			...
			if (opts.onDisallowedCommand) {
				...
				console.log(
					chalk.yellow(`  ⚠ Skipping unallowed criterion check: ${cmd}`),
				);
	...
	private async runTask(task: Task, opts: LoopOptions): Promise<void> {
		...
		const state = await this.stateStore.init(task.task_id);
		const session = await this.sessionStore.init(this.config.provider);
			...
			while (keepRunning) {
				...
				if (result.success) {
					...
					if (summary.passed) {
						...
						console.log(
							chalk.green(`✓ ${task.task_id} done — ${costTracker.summary()}`),
						);
						keepRunning = false;
					} else {
						state.current_status = stateMachine.transition(
						...
						if (this.retrier.shouldRetry(state)) {
							...
							console.log(
								chalk.yellow(
									`  ↺ Retry ${attempt}/${this.config.max_retries}...`,
								),
							);
							...
							console.log(
								chalk.red(
									`✗ ${task.task_id} failed after ${attempt} attempt(s)`,
								),
							);
							keepRunning = false;
							throw new Error(
								`Task ${task.task_id} failed verification: ${errors.join("; ")}`,
							);
						}
					}
				} else {
					...
					if (limitEvent && opts.onLimitHit) {
						...
						const newProvider = await opts.onLimitHit(
							activeProvider,
							limitEvent.message,
						);
						if (newProvider && newProvider !== activeProvider) {
							console.log(
								chalk.yellow(`  → Switching to ${chalk.bold(newProvider)}...`),
							);
							await this.sessionStore.switchProvider(
								newProvider,
								limitEvent.message,
							);
					...
					if (this.retrier.shouldRetry(state)) {
						...
						console.log(chalk.red(`✗ ${task.task_id} failed: ${errorMessage}`));
						keepRunning = false;
						throw new Error(`Task ${task.task_id} failed: ${errorMessage}`);
					}
				}
			}
		} finally {
