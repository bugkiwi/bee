...
export class TaskExecutor {
  ...
  async execute(
    ...
    if (this.config.use_plugins) {
      try {
        ...
        if (contextFiles.length > 0) {
          ...
          enrichedTask = { ...task, _enriched_prompt: enrichedPrompt } as Task & {
    ...
    if (result.tokens_input !== undefined || result.tokens_output !== undefined) {
...
function createStreamPrinter(provider: string): StreamCallback {
  ...
  return (event: ProviderEvent) => {
    switch (event.type) {
      case "text": {
        const e = event.parsed as { message?: { content?: Array<{ type: string; text?: string }> } };
      ...
      case "result": {
        const e = event.parsed as { cost_usd?: number; total_input_tokens?: number; total_output_tokens?: number };
        const parts: string[] = [];
        ...
        if (parts.length) console.log(`${prefix} ${chalk.gray(parts.join(" "))}`);
        break;
      }
      case "error": {
